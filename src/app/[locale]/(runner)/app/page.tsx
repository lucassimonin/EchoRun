  ...
  return (
    <main className="mx-auto max-w-5xl px-5 py-12 sm:px-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <ButtonLink href="/app/races/new">{t('newRace')}</ButtonLink>
      </div>

      {list.length === 0 ? (
        <Surface className="mt-10 py-16 text-center">
          <p className="text-[17px] font-bold uppercase text-charcoal">{t('emptyTitle')}</p>
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="mt-2 font-mono text-[12px] uppercase tracking-[0.02em] text-black/70">
                      {formatRaceDate(race.race_date, locale)}
                      {' · '}
                      <span className="tnum">
                        {race.mode === 'time'
                          ? formatClock(race.duration_s ?? 0, locale)
                          : formatDistance(race.distance_m, locale)}
                      </span>
                      {' · '}
                      <span className="tnum">{count}</span> {t('messages', { count })}
                    </p>
                  </div>
                  <span
                    aria-hidden
