...
export function NewRaceForm() {
  const t = useTranslations('NewRaceForm');
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!res.ok || 'error' in json) {
        setError('error' in json ? json.error : t('errorCreateFailed'));
      ...
      setError(t('errorCreateInterrupted'));
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <Surface>
      ...
      <div className="grid grid-cols-3 gap-2 rounded-lg border-[3px] border-black bg-off p-2">
        {(
          [
            { value: 'file', label: t('modeFileLabel'), hint: t('modeFileHint') },
            { value: 'draw', label: t('modeDrawLabel'), hint: t('modeDrawHint') },
            { value: 'time', label: t('modeTimeLabel'), hint: t('modeTimeHint') },
          ] as const
        ).map((option) => (
          <button
            key={option.value}
      ...
      {inputMode === 'file' ? (
          ...
          {file ? (
            <>
              ...
              <p className="mt-1 font-mono text-[12px] text-black/60">
                {t('fileSize', { size: (file.size / 1024).toFixed(0) })}
              </p>
              <button
                type="button"
                onClick={() => pick(null)}
            ...
            <>
              <p className="text-[14px] text-charcoal-muted">{t('dropHere')}</p>
              <Button
                variant="secondary"
                size="sm"
                className="mt-4"
        ...
        <div className="mt-5">
          ...
          <div className="mt-4 flex items-center gap-3">
            <span className="text-[12.5px] text-charcoal-faint">{t('orSpecify')}</span>
            <TextInput
              id="race-duration"
              type="number"
              min={5}
            ...
            />
            <span className="text-[13px] text-charcoal-muted">{t('minutes')}</span>
          </div>

          {durationMin !== null && durationMin >= 5 ? (
            <p className="mt-3 text-[13px] font-bold text-black">
              {t('raceDurationSummary', { clock: formatClock(durationMin * 60, locale) })}
            </p>
          ) : null}
        </div>
      )}
      ...
      <div className="mt-6 space-y-5">
        <Field label={t('nameLabel')} htmlFor="race-name">
          <TextInput
            id="race-name"
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder={inputMode === 'time' ? t('namePlaceholderTime') : inputMode === 'draw' ? t('namePlaceholderDraw') : t('namePlaceholderFile')}
            maxLength={120}
          />
        </Field>
          ...
          {debug ? (
            <details className="rounded-md border-2 border-black/20 bg-black/[0.03] px-3 py-2">
              ...
              <pre className="mt-2 overflow-x-auto whitespace-pre-wrap break-words font-mono text-[11px] leading-relaxed text-charcoal-muted">
                {debug.context}
                {'\n'}{t('debugCode')} {debug.code}
                {debug.message ? '\n' + debug.message : ''}
                {debug.details ? '\n' + t('debugDetails') + ' ' + debug.details : ''}
                {debug.hint ? '\n' + t('debugHint') + ' ' + debug.hint : ''}
              </pre>
            </details>
          ) : null}
        </div>
      ...
      >
        {busy ? (inputMode === 'file' ? t('analyzing') : t('creating')) : t('createRace')}
      </Button>
    </Surface>
  );
}
