 ...
 */
export async function loadPublicRace(slug: string, locale: string): Promise<PublicRace | null> {
  ...
  const [{ data: taken }, settings, t] = await Promise.all([
    supabase
      .from('audio_messages')
    ...
    getAppSettings(),
    getTranslations({ locale, namespace: 'PublicRacePage' }),
  ]);
  ...
  return {
    ...
    status: race.status,
    mode: race.mode,
    runner_name: profile?.display_name?.trim() || t('runnerFallback'),
