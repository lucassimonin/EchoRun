...
export function ContributorFlow({ race, closed, adClientId, adSlotId }: ContributorFlowProps) {
  const t = useTranslations('ContributorFlow');
  ...
  const submitName = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok || 'error' in json) {
        setError('error' in json ? json.error : t('cannotContinue'));
        return;
      }
  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      setNotice(t('messageSent', { name: race.runner_name }));
    } catch {
      setError(t('sendInterrupted'));
    } finally {
      setSending(false);
    }
  }, [identity, isTimeMode, recording, selected, selectedTimeS, race.runner_name, t]);
        ...
        <>
          ...
          {phase === 'identify' ? (
            <Surface className="mt-8 animate-rise">
              <StepHeader index={1} title={t('step1Title')} />
              <p className="mb-5 text-[13.5px] leading-relaxed text-charcoal-muted">
                {t.rich('step1Body', {
                  name: race.runner_name,
                  draft: sanitizeName(nameDraft) || '…',
                  strong: (c) => <strong>{c}</strong>,
                })}
              </p>
              <Field label={t('firstNameLabel')} htmlFor="contributor-name" error={error}>
                <TextInput
                  id="contributor-name"
                  value={nameDraft}
              ...
              >
                {identifying ? t('identifying') : t('continueBtn')}
              </Button>
            </Surface>
          ) : null}
            ...
            <>
              <div className="mt-8 flex items-center justify-between gap-4">
                <p className="text-[13.5px] text-charcoal-muted">
                  {t.rich('greeting', {
                    name: identity.name,
                    strong: (c) => <strong className="font-medium text-charcoal">{c}</strong>,
                  })}
                </p>
                ...
                <>
                  {isTimeMode && race.duration_s ? (
                    <Surface className="mt-4 animate-rise">
                      <StepHeader index={2} title={t('step2TimeTitle')} />
                      <p className="mb-2 text-[13.5px] leading-relaxed text-charcoal-muted">
                        {t('step2TimeBody', { name: race.runner_name })}
                      </p>
                      <Timeline
                    ...
                    <Surface className="mt-4 overflow-hidden p-0 animate-rise">
                      <div className="p-6 pb-4">
                        <StepHeader index={2} title={t('step2DistanceTitle')} />
                          ...
                          <>
                            <div>
                              <p className="text-[14px] font-medium text-charcoal">
                                {t('selectedKm', {
                                  km: (selected.distanceAlongM / 1000)
                                    .toFixed(1)
                                    .replace('.', locale === 'en' ? '.' : ','),
                                })}
                              </p>
                              <p className="mt-0.5 text-[12px] text-charcoal-faint">
                                {t('triggerRadius', { m: TRIGGER_RADIUS_M })}
                              </p>
                            </div>
                  ...
                  >
                    <StepHeader index={3} title={t('step3Title')} />
                    ...
                    {recording ? (
                      <>
                        ...
                        <Button fullWidth size="lg" onClick={() => void send()} disabled={!canSend}>
                          {sending ? t('sending') : t('sendButton', { name: race.runner_name })}
                        </Button>
                      </>
                    ) : null}
...
function PersonalBadge({ state }: { state: ContributorPostState }) {
  ...
  return (
    <Badge tone={tone}>
      {left > 0
        ? t('badgeRemaining', { count: left })
        : t('badgeAllSent', { count: state.personalCap })}
    </Badge>
  );
}
...
function FullNotice({
  ...
  return (
    <Surface className="mt-4 animate-rise">
      {personalFull ? (
        <>
          <Badge tone="matcha">{t('fullBadge', { count: state.personalUsed })}</Badge>
          ...
          <p className="mt-2.5 text-[14px] leading-relaxed text-charcoal-muted">
            {t('fullBody', { count: state.personalCap, name: runnerName })}
          </p>
        </>
      ) : (
        <>
          <Badge tone="clay">{t('fullRaceBadge')}</Badge>
          <h2 className="mt-4 text-[20px] font-semibold tracking-[-0.02em] text-charcoal">
            {t('fullRaceTitle')}
          </h2>
          <p className="mt-2.5 text-[14px] leading-relaxed text-charcoal-muted">
            {t('fullRaceBody', { name: runnerName })}
          </p>
        </>
      )}
    </Surface>
